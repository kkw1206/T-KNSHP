...
public class HashImplSha256 implements Hash {
    private String defaultCharset = "UTF-8";
    private String algorithm = "SHA-256";

    public HashImplSha256() {
    }
    ...
    public String getSha256NoSalt(byte[] input, String key) throws NoSuchAlgorithmException, InvalidKeyException, IllegalStateException, UnsupportedEncodingException {
        if (input != null && key != null && input.length >= 1 && key.length() >= 1) {
            MessageDigest sh = MessageDigest.getInstance(this.algorithm);
            sh.update(input);
            byte[] encBytes = sh.digest();
